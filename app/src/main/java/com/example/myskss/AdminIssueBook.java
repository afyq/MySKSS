...
public class AdminIssueBook extends AppCompatActivity {
    ...
    private boolean getUser() {
        db.collection("Users").whereEqualTo("card_number", Integer.parseInt(editCardNo1.getEditText().getText().toString().trim())).get().addOnCompleteListener(new OnCompleteListener<QuerySnapshot>() {
            @Override
    ...
    private void issueBook() {
        ...
        if (getBook() & getUser()) {
            ...
            db.document("Users/" + U.getEmail()).set(U).addOnCompleteListener(new OnCompleteListener<Void>() {
                @Override
